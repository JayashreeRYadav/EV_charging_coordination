...
@variable(m, S);
# @objective(m, Min, maximum(abs(D[t] + sum(p[n, t] for n = 1:N)) for t = 1:T));
@objective(m, Min, S)
# S = maximum(abs(D[t] + sum(p[n, t] for n = 1:N)) for t = 1:T);
# @constraint(m, (D[t] + sum(p[n, t] for n = 1:N) <= S for t = 1:T))
@constraint(m, p .<= pub);
@constraint(m, p .>= plb);
...
for t = 1:T
    ...
    @constraint(m, (D[t] + sum(p[n, t] for n = 1:N) <= S))
end
S = maximum(abs(D[t] + sum(p[n, t] for n in 1:N)) for t in 1:T)
...
if solveFlag==0
    matwrite("Data_infnorm_Gurobi.mat", Dict("ev_charge_Gurobi" => p_value, "D" => D));
elseif solveFlag == 1
    matwrite("Data_infnorm_Ipopt.mat", Dict("ev_charge_ipopt" => p_value,  "D" => D));
end
plot(title="EV charging Distribution infinity norm", ylabel="Value", xlabel="time step")
...
filename = "solution_matrices_infnorm.mat"
